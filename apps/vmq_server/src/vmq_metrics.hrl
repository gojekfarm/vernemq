...
-define(REASON_NOT_AUTHORIZED, not_authorized).
-define(REASON_NORMAL_DISCONNECT, normal_disconnect).
-define(REASON_SESSION_TAKEN_OVER, session_taken_over).
-define(REASON_ADMINISTRATIVE_ACTION, administrative_action).
-define(REASON_DISCONNECT_KEEP_ALIVE, disconnect_keep_alive).
-define(REASON_DISCONNECT_MIGRATION, disconnect_migration).
-define(REASON_BAD_AUTHENTICATION_METHOD, bad_authentication_method).
-define(REASON_REMOTE_SESSION_TAKEN_OVER, remote_session_taken_over).
-define(REASON_MQTT_CLIENT_DISCONNECT, mqtt_client_disconnect).
-define(REASON_RECEIVE_MAX_EXCEEDED, receive_max_exceeded).
-define(REASON_PROTOCOL_ERROR, protocol_error).
-define(REASON_PUBLISH_AUTH_ERROR, publish_not_authorized_3_1_1).
-define(REASON_INVALID_PUBREC_ERROR, invalid_pubrec_error).
-define(REASON_INVALID_PUBCOMP_ERROR, invalid_pubcomp_error).
-define(REASON_UNEXPECTED_FRAME_TYPE, unexpected_frame_type).
-define(REASON_EXIT_SIGNAL_RECEIVED, exit_signal_received).
-define(REASON_TCP_CLOSED, tcp_closed).
-define(REASON_NORMAL, nomal).
-define(REASON_UNSPECIFIED, unspecified).
