...
init(Peer, Opts, #mqtt_connect{keep_alive=KeepAlive,
  ...
  lager:error("fsm initted for ~p", [SubscriberId]),
...
data_in(Data, SessionState, OutAcc) ->
  case vmq_parser_mqtt10:parse(Data, max_msg_size()) of
    more ->
...
serialise([[F|T]|Frames], Acc) ->
  serialise([T|Frames], [vmq_parser_mqtt10:serialise(F)|Acc]).
...
connected(#mqtt_publish{message_id=MessageId, topic=Topic,
  ...
  lager:error("publish received retry=~p qos=~p msg=~p, id=~p", [Retryable, QoS, Payload, MessageId]),
  ...
  case Ret of
    {error, not_allowed} ->
      ...
      terminate(publish_not_authorized_10, State);
...
connected(#mqtt_puback{message_id=MessageId}, #state{waiting_acks=WAcks, subscriber_id = SubscriberId, username = Username} = State) ->
  ...
  case maps:get(MessageId, WAcks, not_found) of
    #vmq_msg{routing_key = Topic, payload = Payload, retain=IsRetain, qos=QoS} ->
      lager:error("puback received id=~p, payload=~p", [MessageId, Payload]),
...
terminate_reason(publish_not_authorized_10) -> normal;
...
handle_waiting_acks_and_msgs(State) ->
  ...
  lager:error("Msgs to be delivered next time=~p", [MsgsToBeDeliveredNextTime]),
...
handle_messages([#deliver{} = Obj|Rest], Frames, PubCnt, State, Waiting) ->
  case check_in_flight(State) of
    ...
    false ->
      % now all kinds of messages are constrained by max_flight config.
...
prepare_frame(#deliver{qos=QoS, msg_id=MsgId, msg=Msg}, State) ->
  ...
  Frame = #mqtt_publish{message_id=OutgoingMsgId,
    ...
    dup=IsDup,
    payload=NewPayload,
    retryable = is_message_retryable(Retryable, MsgQoS)},
  case NewQoS of
    0 ->
      case is_message_retryable(Retryable, MsgQoS) of
        true ->
          {Frame, State1#state{
...
is_message_retryable(Retryable, 0) ->
  Retryable;
is_message_retryable(_, _) ->
  true.
