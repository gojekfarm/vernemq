...
expect_packet(Transport, Socket, _Name, Expected, Timeout) ->
  ...
  case Transport:recv(Socket, RLen, Timeout) of
    ...
    {ok, Different} ->
      ...
      {ExpectedFrame, <<>>} = vmq_parser_mqtt10:parse(Expected),
      {DifferentFrame, <<>>} = vmq_parser_mqtt10:parse(Different),
...
gen_connect(ClientId, Opts) ->
  vmq_parser_mqtt10:gen_connect(ClientId, Opts).

gen_connack() ->
  gen_connack(0).
gen_connack(RC) ->
  vmq_parser_mqtt10:gen_connack(RC).
gen_connack(SessionPresent, RC) ->
  vmq_parser_mqtt10:gen_connack(SessionPresent, RC).

gen_publish(Topic, Qos, Payload, Opts) ->
  vmq_parser_mqtt10:gen_publish(Topic, Qos, Payload, Opts).

gen_puback(Mid) ->
  vmq_parser_mqtt10:gen_puback(Mid).

gen_pubrec(Mid) ->
  vmq_parser_mqtt10:gen_pubrec(Mid).

gen_pubrel(Mid) ->
  vmq_parser_mqtt10:gen_pubrel(Mid).

gen_pubcomp(Mid) ->
  vmq_parser_mqtt10:gen_pubcomp(Mid).

gen_subscribe(MId, Topics) ->
  vmq_parser_mqtt10:gen_subscribe(MId, Topics).

gen_subscribe(MId, Topic, Qos) ->
  vmq_parser_mqtt10:gen_subscribe(MId, Topic, Qos).

gen_suback(Mid, Qos) ->
  vmq_parser_mqtt10:gen_suback(Mid, Qos).

gen_unsubscribe(MId, Topic) ->
  vmq_parser_mqtt10:gen_unsubscribe(MId, Topic).

gen_unsuback(MId) ->
  vmq_parser_mqtt10:gen_unsuback(MId).

gen_pingreq() ->
  vmq_parser_mqtt10:gen_pingreq().

gen_pingresp() ->
  vmq_parser_mqtt10:gen_pingresp().

gen_disconnect() ->
  vmq_parser_mqtt10:gen_disconnect().
