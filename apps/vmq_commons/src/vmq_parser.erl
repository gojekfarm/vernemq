...
serialise_topics(?SUBSCRIBE = Sub, [#mqtt_subscribe_topic{
  ...
  qos = QoS,
  non_persistence=NP,
  non_retry=NR}|Rest], Acc) ->
    NonRetry = to_int(NR),
    NonPersistence = to_int(NP),
    serialise_topics(Sub, Rest, [utf8(vmq_topic:unword(Topic)), <<0:4, NonRetry:1, NonPersistence:1, QoS:2>>|Acc]);
...
gen_subscribe(MId, [#mqtt_subscribe_topic{}|_] = Topics) ->
    BinTopics = [E#mqtt_subscribe_topic{topic=ensure_binary(T)}
    || #mqtt_subscribe_topic{topic = T} = E <- Topics],
    iolist_to_binary(serialise(#mqtt_subscribe{topics=BinTopics, message_id=MId})).
gen_subscribe(MId, Topic, QoS) ->
    gen_subscribe(MId, [{Topic, QoS}]).
gen_subscribe(MId, Topic, QoS, NonRetry, NonPersistence) ->
    gen_subscribe(MId, [#mqtt_subscribe_topic{topic=Topic, qos=QoS, non_retry=NonRetry, non_persistence=NonPersistence}]).
